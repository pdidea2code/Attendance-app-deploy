...
const Holiday = () => {
  ...
  if (
    (Array.isArray(permission) && permission.includes('holiday.delete')) ||
    permission.includes('holiday.edit')
  ) {
    columns.push({
      name: '_id',
      label: 'Action',
      options: {
        ...
        customBodyRender: (value) => {
          return (
            <>
              {Array.isArray(permission) && permission.includes('holiday.edit') && (
                <Icons.EditRounded
  ...
  const deleteMultiple = async (selectedRows) => {
    ...
    const confirm = await swal({
      title: 'Are you sure?',
      text: 'Are you sure that you want to delete selected Holiday?',
      icon: 'warning',
      buttons: ['No, cancel it!', 'Yes, I am sure!'],
      dangerMode: true,
    })
  ...
  const options = {
    customToolbarSelect: (selectedRows) =>
      ...
      permission.includes('holiday.delete') && <SelectedRowsToolbar selectedRows={selectedRows} />,
  }
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <ToastContainer />
          {Array.isArray(permission) && permission.includes('holiday.add') && (
            <div className="right-text">
              <Button
                variant="contained"
                size="medium"
                className="AddButton"
                onClick={() => navigate('/holidayform')}
              >
                Add
              </Button>
            </div>
